	...
	if cfg.main.gm then
		...
		if isKeysDown(facceptform.v) and not sampIsChatInputActive() and not sampIsDialogActive() and not isPauseMenuActive() and not isSampfuncsConsoleActive() and cfg.main.trueform then
			...
			lua_thread.create(function()
				...
				wait (502)
				sampAddChatMessage("{00BFFF}[Ikona-TOOLS] - Успешно принятая форма от: " .. admin_nick .. "{FFFFFF}")	
				acceptforma = "true"
			end)
			end
		end
...
if imgui.Button(u8'Указать свою причину', imgui.ImVec2(-1, 20)) then
   sampSetChatInputEnabled(true)
   sampAddChatMessage(tag..'На Arizona RP данная функция не работает!', -1)
   imgui.CloseCurrentPopup()
end
if imgui.Button(u8'Закрыть', imgui.ImVec2(140, 20)) then
imgui.CloseCurrentPopup()
...
if not isGamePaused() then	
	...
	if text:find("%[A] (Sasha_Gozhelnikov)%[(%d+)%]: bot check updates") then
		...
		anickname, aid = text:match("%[A] (Sasha_Gozhelnikov)%[(%d+)%]: bot obnova")
				sampSendChat('/a новая версия скрипта, установить: vk.com/ikonacoder')
	end
...
sampAddChatMessage(tag..'Разработчик: {FF69B4}Иконка Луас *ikonka luac', -1)
sampAddChatMessage(tag..'Для получения дополнительной информации по скрипту, посетите сайт: {FF69B4}blast.hk/members/542334/', -1)
sampAddChatMessage(tag..'Этот скрипт работает только на: {FF69B4}ARIZONA RP', -1)
end
