...
function update()
    local raw = "https://github.com/TestSender/UpdateTestTools/raw/refs/heads/main/AutoUpdate.json"
    local dlstatus = require('moonloader').download_status
...
locations = {
{u8'Àâòîñàëîí ËÑ', -484.0909,-557.7690,25.5634},
{u8'Ôåðìà íîâè÷êîâ', -86.9788,88.2501,3.1172},
{u8'Àâòîñàëîí ËÂ', 947.2376,2185.3940,10.8203},
{u8'Ñïàâí LS ¹2', 2219.6565,-1146.2649,25.7819},
{u8'Öåíòðàëüíûé ðûíîê', 1125.7731,-1425.4581,15.7969},
{u8'ÂàéíÂóä', 1383.2190,-894.9438,37.1505},
{u8'Èíôîðìàöèîííûé öåíòð', -2238.2156,575.6461,35.1719},
{u8'Àììî LS', 1363.6930,-1279.8029,13.5469},
{u8'Ñïàâí LS ¹1', 1759.4504,-1900.2614,13.6300},
{u8'Çàâîä äëÿ ÌÏ', 2508.1938,2767.6387,10.8203},
{u8'Øàõòà', 522.3231,885.0934,-37.4623},
{u8'Àâòîñàëîí ÑÔ', -2663.9944,-1.8084,4.3267},
{u8'Êàçèíî ËÂ', 2030.6858,998.3827,10.8203},
{u8'Àâòîçàâîä', 1576.6055,713.8749,10.8203},
{u8'Àâòîñàëîí ÑÔ ïîääåëüíûõ àâòî (íåëåãàëêè)', -2466.1277,2251.1570,4.7997},
{u8'Ñïàâí LV ¹1', 2844.6235,1291.8997,11.3906},
{u8'Ñïàâí LV ¹2', -71.9333,1224.7753,19.6904},
{u8'Ñïàâí SF', -1986.2416,147.1740,27.6875},
{u8'Àâòîáàçàð', -2136.6956,-750.2112,32.0234},
{u8'Ëåñ', -1649.5634,-2237.9788,30.1431},
{u8'Áàíê LV', 2375.4661,2316.0024,8.1406},
{u8'Öåíòðàëüíûé Áàíê', 1469.8745,-1743.1466,13.5469},
{u8'Çàáðîøåííûé àýðîïîðò', 376.8152,2511.9451,16.5445},
{u8'Öåíòð çàíÿòîñòè', 1287.5400,-1275.4004,13.5407}
}
...
function getCapitalLetter(text, mode)
	...
	if mode == 1 then
		string_world = '¨ÉÖÓÊÅÍÃØÙÇÕÚÔÛÂÀÏÐÎËÄÆÝß×ÑÌÈÒÜÁÞ'
	elseif mode == 2 then
		string_world = 'QWERTYUIOPASDFGHJKLZXCVBNM'
	elseif mode == 3 then
		string_world = '¨ÉÖÓÊÅÍÃØÙÇÕÚÔÛÂÀÏÐÎËÄÆÝß×ÑÌÈÒÜÁÞQWERTYUIOPASDFGHJKLZXCVBNM'
	end
...
'Òåïåðü êîíôèã ñêðèïòà õðàíèòñÿ â ïàïêå ñêðèïòà, à íå â ïàïêå config äëÿ âñåãî ìóíëîàäåðà (ïóòü: moonloader/config/AdminToolsKing/AdminTools.ini) '
		},
		['patches'] = {
		['show'] = false,
			['info'] = {
...
pathOffLog = getWorkingDirectory().."\\AdminToolsKing\\Ëîãè\\AutoOpraLogs.txt"
log_full = {}
...
function main()
	...
	if not doesFileExist(getWorkingDirectory() .. '/AdminToolsKing/IMG_4237.png') then
		downloadUrlToFile(ImgForAuthor, getWorkingDirectory() .. '/AdminToolsKing/IMG_4237.png', function (id, status, p1, p2)
			if status == dlstatus.STATUSEX_ENDDOWNLOAD then
				LoadTextureFunc()
				sampAddChatMessage('ÑÊÀ×ÀËËËËËËËË', -1)
			end
		end)
	else
		LoadTextureFunc()
    ...
    sampAddChatMessage('Ñêðèïò çàãðóæåí, âåðñèÿ: '..lastver, -1)
    if thisScript().version ~= lastver then
		sampAddChatMessage('Âûøëî îáíîâëåíèå ñêðèïòà ('..thisScript().version..' -> '..lastver..'). Ñêðèïò îáíîâëÿåòñÿ!', -1)
        update():download()
    end--]]
	...
	{'carnames', function(param)
		if param:find('^%d') and tonumber(param) <= 2 then
		  if tonumber(param) ~= tonumber(cfg.Settings.mode) then
			...
			sampAddChatMessage(string.format('Â äàííûé ìîìåíò èòàê âêëþ÷åíî îòîáðàæåíèå %s [{1E90FF}%d{FFFFFF}].', messageChangeMode[param + 1], param), -1)
		  end
		else
		  sampAddChatMessage(string.format('Òàêîãî ïàðàìåòðà íå ñóùåñòâóåò! Êîëè÷åñòâî äîñòóïíûõ ïàðàìåòðîâ: {1E90FF}%d{FFFFFF}.', #messageChangeMode), -1)
		  for k, v in pairs(messageChangeMode) do
...
local function get_id_by_nick(nick)
	...
	for i = 0, 4 do
		...
		if cfg.main.leaveChecker then 
			...
			renderFontDrawTextAlign(leave, "×åêåð âûõîäîâ: ", cfg.leavePosition.x, cfg.leavePosition.y, -1, cfg.main.aligntwo)
			for k,v in pairs(leavePlayers) do
				y = y + cfg.main.leaveIndent
				renderFontDrawTextAlign(leave, "{"..leavePlayers[k].playerColor.."}["..leavePlayers[k].timeQuit.."] "..leavePlayers[k].nickname.."["..leavePlayers[k].id.."] îòêëþ÷èëñÿ. Ïðè÷èíà: "..leavePlayers[k].reason, x, y, -1, cfg.main.aligntwo)
		...
		if cfg.main.connectChecker then
			...
			renderFontDrawTextAlign(connect, "×åêåð âõîäîâ: ", cfg.connectPosition.x, cfg.connectPosition.y, -1, cfg.main.alignthree)
			for k,v in pairs(connectPlayers) do
				y = y + cfg.main.connectIndent
				renderFontDrawTextAlign(connect, "{"..connectPlayers[k].playerColor.."}["..connectPlayers[k].timeJoin.."] "..connectPlayers[k].nickname.."["..connectPlayers[k].id.."] ïîäêëþ÷èëñÿ.", x, y, -1, cfg.main.alignthree)
		...
		if cfg.main.adminsChecker then
			...
			--renderFontDrawTextAlign(admins, "×åêåð àäìèíèñòðàöèè: ", cfg.adminsPosition.x, cfg.adminsPosition.y, -1, cfg.main.adminsAlign)
			for k,v in pairs(adminsPlayers) do
				y = y + cfg.main.adminsotschecker
				--renderFontDrawTextAlign(admins, "{"..adminsPlayers[k].playerColor.."}["..adminsPlayers[k].timeJoin.."] "..adminsPlayers[k].nickname.."["..adminsPlayers[k].id.."] ïîäêëþ÷èëñÿ.", x, y, -1, cfg.main.adminsAlign)
		...
		if cfg.main.regChecker then
			...
			renderFontDrawTextAlign(reg, "×åêåð ðåãèñòðàöèé: ", cfg.RegPosition.x, cfg.RegPosition.y, -1, cfg.main.alignfour)
			for k,v in pairs(regPlayers) do
				y = y + cfg.main.regIndent
				renderFontDrawTextAlign(reg, "{FFFFFF}["..regPlayers[k].timeReg.."] "..regPlayers[k].nickname.."["..regPlayers[k].id.."] çàðåãèñòðèðîâàëñÿ | IP: "..regPlayers[k].ip1..".xx.xxx."..regPlayers[k].ip4.."", x, y, -1, cfg.main.alignfour)
	  ...
	  if isKeyCheckAvailable() and isKeysDown(fopenmap.v) then
		...
		if reduceZoom then
		  writeMemory(menuPtr + 0x64, 4, representFloatAsInt(300.0), false)
			...
			renderFontDrawText(my_font, 'Èçìåíåíèå ñêîðîñòè: {E78284}'..klavisha..' {84A6E7}è {E78284}'..klavisha1..'\n{84A6E7}Òåêóùàÿ ñêîðîñòü: {E78284}'..boost, cx + 500, windowCoordinates[2] - 32, 0xFF84A6E7)
		...
		sampAddChatMessage(tag..'Àâòîëîâëÿ ðåïîðòà âûêëþ÷åíà, òàê êàê âû íàõîäèòåñü â ñòàòóñå "Ïîäêëþ÷åíèå ê ñåðâåðó"', -1)
		cfg.main.lovlyareporta = false
		end
		
		if sampGetGamestate() ~= 3 and cfg.main.autopiar then 
		sampAddChatMessage(tag..'Àâòîðåêëàìà âûêëþ÷åíà, òàê êàê âû íàõîäèòåñü â ñòàòóñå "Ïîäêëþ÷åíèå ê ñåðâåðó"', -1)
	...
	if cfg.main.gm then
		...
		if not sampIsChatInputActive() and not sampIsDialogActive() and not isSampfuncsConsoleActive() and isPlayerPlaying(playerHandle) then
			...
			sampAddChatMessage(tag..'Íåëüçÿ âêëþ÷èòü àâòîëîâëþ ðåïîðòà ïðè ïîäêëþ÷åíèè ê ñåðâåðó!', -1)
				...
				sampAddChatMessage(tag..'Àâòîëîâëÿ ðåïîðòà '..(cfg.main.lovlyareporta and 'âêëþ÷åíà' or 'âûêëþ÷åíà'), -1)
			end
			end
		end
		...
		if cfg.main.airbrake then 
			...
			printStringNow("~w~IKONKA "..(enAirBrake and "~b~OFF" or "~g~ON"), 2000)
		...
		renderFontDrawText(my_font, 'Èçìåíåíèå ñêîðîñòè: {E78284}'..klavisha..' {84A6E7} è {E78284}'..klavisha1..'\n{84A6E7}Òåêóùàÿ ñêîðîñòü: {E78284}'..cfg.main.speed_airbrake, cx + 500, windowCoordinates[2] - 32, 0xFF84A6E7)
		...
		if isKeysDown(facceptform.v) and not sampIsChatInputActive() and not sampIsDialogActive() and not isPauseMenuActive() and not isSampfuncsConsoleActive() and cfg.main.trueform then
			...
			lua_thread.create(function()
				...
				status("true", 1)	
				sampAddChatMessage("[AdminTools] {"..cfg.main.twocolorforms.."}Ôîðìà >> {"..cfg.main.colorforms.."}/"..cmd.." "..paramssss.." {"..cfg.main.twocolorforms.."}<< âûäàíà!", "0xFF"..cfg.main.colorforms)
				local f = io.open(getGameDirectory().."//moonloader//AdminToolsKing//AcceptForms.txt", 'a')
				f:write('['..os.date("%d.%m.%Y %X", os.time())..'] Ôîðìà: /'..cmd..' '..paramssss..' | Îòïðàâèòåëü: '..admin_nick..' | Òèï ïðèíÿòèÿ ôîðìû: âðó÷íóþ\n')
				f:close()
				...
				autoformaq = "false"
				wait (502)
				sampSendChat("/a [IKONA-TOOLS] ~ Forma [+]")
				wait (502)
				sampAddChatMessage("{00BFFF}[Ikona-TOOLS] - Óñïåøíî ïðèíÿòàÿ ôîðìà îò: " .. admin_nick .. "{FFFFFF}")	
				acceptforma = "true"
			end)
			end
		end
...
local X, Y = getScreenResolution()
	local colours = {u8'Ðîçîâûé', u8'Çåë¸íûé', u8'Êðàñíûé', u8'Ôèîëåòîâûé', u8'×¸ðíûé', u8"Áåëûé"}
	local binds = {u8'Çàùèòà àêêàóíòà', u8'Ìíîæèòåëü äîíàòà', u8'Ïðàâèëà ñåðâåðà', u8'Äèñêîðä', u8'Ïàáëèê', u8'Ñîáñòâåííûé ¹1', u8'Ñîáñòâåííûé ¹2', u8'Ñîáñòâåííûé ¹3', u8'Ñîáñòâåííûé ¹4'}
	if MainWindow.v then
		imgui.SetNextWindowPos(imgui.ImVec2(sw / 2, sh / 2), imgui.Cond.FirstUseEver, imgui.ImVec2(0.5, 0.5))
		imgui.SetNextWindowSize(imgui.ImVec2(900, 550), 1)
		imgui.Begin(fa.ICON_FA_TOOLS..u8' ARZ RP | TOOLS '..fa.ICON_FA_TOOLS, MainWindow, imgui.WindowFlags.NoResize + imgui.WindowFlags.NoSavedSettings) 	
	   ...
	   imgui.Hint('saves', u8'Ñîõðàíèòü íàñòðîéêè')
		...
		if menusel.v == 1 then
			...
			imgui.TextDisabled(fa.ICON_FA_QUESTION_CIRCLE..u8'')
			imgui.PopFont()
			imgui.Hint('helpa13', u8'Àíàëîã ìåòëû, òîëüêî â ðàçû áûñòðåå\nÂêë./âûêë.: R.SHIFT\nÓñêîðåíèå: Num +\nÇàìåäëåíèå:Num -')
			...
			imgui.TextDisabled(fa.ICON_FA_QUESTION_CIRCLE..u8'')
			imgui.PopFont()
			imgui.Hint('helpa3', u8'Ââåäèòå ID ôðàêöèè â êîòîðóþ âû áóäåòå âñòóïàòü ïðè àâòîðèçàöèè\nÔóíêöèÿ äîñòóïíà ñ 4+ LVL')
			...
			imgui.TextDisabled(fa.ICON_FA_QUESTION_CIRCLE..u8'')
			imgui.PopFont()
			imgui.Hint('helpa2', u8'Ââåäèòå ID ñêèíà êîòîðûé áóäåò âûäàâàòüñÿ ïðè àâòîðèçàöèè\nÔóíêöèÿ äîñòóïíà ñ 4+ LVL')
			...
			imgui.TextDisabled(fa.ICON_FA_QUESTION_CIRCLE..u8'')
			imgui.PopFont()
			imgui.Hint('helpa1', u8'Äëÿ ðàáîòû äàííîé ôóíêöèè óêàæèòå ñâîé ïàðîëü îò àêêàóíòà â ñîîòâåòñòâóþùåì ïóíêòå.\nÏðè ïîäêëþ÷åíèè íà ñåðâåð ñêðèïò ñàì ìîìåíòàëüíî ââåäåò ïàðîëü')
			...
			imgui.TextDisabled(fa.ICON_FA_QUESTION_CIRCLE..u8'')
			imgui.PopFont()
			imgui.Hint('helpa113', u8'ÈÏû â ñîîáùåíèÿõ áóäóò çàìåíÿòüñÿ òàêèì îáðàçîì: 93.õõ.õõ.186\nÏîëåçíàÿ ôóíêöèÿ, âåäü íà îïðàõ áîëüøå íå áóäåò ÈÏîâ èãðîêîâ/àäìèíîâ')
			if imgui.ToggleButton(u8'Íîï F5', imgui.ImBool(cfg.main.nopf5)) then 
				...
				sampAddChatMessage('[Èíôîðìàöèÿ] Ïîñëå ïåðåçàõîäà â èãðó êëàâèøà F5 ñíîâà áóäåò ðàáîòàñïîñîáà', 14628149)
				end
			end 
			imgui.SameLine()
			imgui.PushFont(minimalfont)
			...
			--end 
			--if imgui.ToggleButton(u8'×åêåð àäìèíèñòðàöèè [OFF SISTEM]', imgui.ImBool(cfg.main.leaveChecker)) then 
			...
			--imgui.TextDisabled(fa.ICON_FA_QUESTION_CIRCLE..u8'')
			--imgui.PopFont()
			--imgui.Hint('helpa14', u8'×åêåð àäìèíèñòðàöèè, âûâîäèò ñïèñîê àäìèíîâ íà ýêðàí.')
			...
			imgui.TextDisabled(fa.ICON_FA_QUESTION_CIRCLE..u8'')
			imgui.PopFont()
			imgui.Hint('antitps ', u8'Ïðè âõîäå ïîä àäìèíêó ó âàñ àâòîìàòè÷åñêè áóäåò\nâêëþ÷àòüñÿ /antitp')
			imgui.NextColumn()
			...
			imgui.TextDisabled(fa.ICON_FA_QUESTION_CIRCLE..u8'')
			imgui.PopFont()
			imgui.Hint('heloa388433', u8'Ïðè íàæàòèè êíîïêè > èëè < âû ïåðåéäåòå â ñëåæêó çà ñëåäóþùèì èëè ïðîøëûì èãðîêîì')
			...
			if imgui.ToggleButton(u8'Íîï F4', imgui.ImBool(cfg.main.nopf4)) then 
				...
				sampAddChatMessage('[Èíôîðìàöèÿ] Ïîñëå ïåðåçàõîäà â èãðó êëàâèøà F4 ñíîâà áóäåò ðàáîòàñïîñîáà', 14628149)
				end
			end 
			imgui.SameLine()
			imgui.PushFont(minimalfont)
			imgui.TextDisabled(fa.ICON_FA_QUESTION_CIRCLE..u8'')
			imgui.PopFont()
			imgui.Hint('helpa21', u8'Îòêëþ÷àåò ðàáîòó êëàâèøè F4, ïðè ÷àñòîì ïîïàäàíèè â áîëüêó îòêëþ÷èòå ôóíêöèþ è ïåðåçàéäèòå')
			...
			imgui.TextDisabled(fa.ICON_FA_QUESTION_CIRCLE..u8'')
			imgui.PopFont()
			imgui.Hint('helpa26', u8'Äàííàÿ ôóíêöèÿ ëîããèðóåò âñå ïðèíÿòûå ôîðìû â îòäåëüíûé ôàéë, äàáû ñîêðàòèòü ñëó÷àè ïîäñòàâû')
			...
			imgui.TextDisabled(fa.ICON_FA_QUESTION_CIRCLE..u8'')
			imgui.PopFont()
			imgui.Hint('helpa27', u8'Ïðè âõîäå â àäìèíêó ñêðèïò áóäåò àâòîìàòè÷åñêè óñòàíàâëèâàòü óðîâåíü àäìèíèñòðàòîðà íà ñåðâåðíûé')
			...
			imgui.TextDisabled(fa.ICON_FA_QUESTION_CIRCLE..u8'')
			imgui.PopFont()
			imgui.Hint('helpa2311384122381', u8'Ïðè ñòðåëüáå ó âàñ àâòîìàòè÷åñêè áóäåò ñêðîëëèòüñÿ îðóæèå, òåì ñàìûì îòñóòñòâîâàòü ïåðåçàðÿäêà')
			...
			imgui.TextDisabled(fa.ICON_FA_QUESTION_CIRCLE..u8'')
			imgui.PopFont()
			imgui.Hint('helpa1122311384122381', u8'Ïðè ïðîïèñûâàíèè /getip ñêðèïò àâòîìàòè÷åñêè áóäåò îïðåäåëÿòü ìåñòîïîëîæåíèå è âûâîäèòü â /a ëèáî ëîêàëüíûé ÷àò')
			...
			imgui.TextDisabled(fa.ICON_FA_QUESTION_CIRCLE..u8'')
			imgui.PopFont()
			imgui.Hint('warningtkss', u8'Åñëè èãðîê óáüåò ñâîåãî íàïàðíèêà ïî îðãàíèçàöèè, òî ñêðèïò îïîâåñòèò âàñ îá ýòîì â ÷àòå')
			...
			imgui.TextDisabled(fa.ICON_FA_QUESTION_CIRCLE..u8'')
			imgui.PopFont()
			imgui.Hint('spectatekillaa', u8'Ïðè íàæàòèè ïî íèêíåéìó èãðîêà â êèëë-ëèñòå\nâû àâòîìàòè÷åñêè ïåðåéäåòå â ñëåæêó çà íèì')
			imgui.Columns(1)
			imgui.Separator()
...
imgui.Separator()
	...
	imgui.Text(u8'Îòêðûòü ìåíþ âûäà÷è íàêàçàíèé')
		...
		if menusel.v == 5 then
			...
			sampAddChatMessage(tag..'Çàéäèòå â ðåêîí äëÿ ïîëó÷åíèÿ âîçìîæíîñòè èçìåíåíèÿ äàííîãî ïàðàìåòðà.', -1)
			end
			end
			...
			sampAddChatMessage(tag..'Äëÿ èçìåíåíèÿ äàííîãî ïàðàìåòðà ó âàñ äîëæíà áûòü îòêðûòà ïàíåëü î÷åðåäè ñëåæêè.', -1)
			end
			end
			...
			if imgui.Button(fa.ICON_FA_ARROWS_ALT..u8'##saddsaejjadd') then
				if PunishWindow.v then
					...
					--MainWindow.v = false
					--sampAddChatMessage(tag..'Äëÿ ïîäòâåðæäåíèÿ èçìåíåíèÿ íàæìèòå ÍÅÅÅ 1, ÷òîáû îòìåíèòü ñîõðàíåíèå - 2.', -1)
				else
					sampAddChatMessage(tag..'Çàéäèòå â ðåêîí äëÿ ïîëó÷åíèÿ âîçìîæíîñòè èçìåíåíèÿ äàííîãî ïàðàìåòðà.', -1)
				end
			end
			...
			sampAddChatMessage(tag..'Äëÿ èçìåíåíèÿ äàííîãî ïàðàìåòðà ó âàñ äîëæíà áûòü âêëþ÷åíà èíôîðìàöèîííàÿ ïàíåëü.', -1)
			end
			end
			imgui.Separator()
			imgui.EndChild() 		
	...
	if imgui.Button(u8("Äîáàâèòü êíîïêó îòâåòà â ðåïîðò"), imgui.ImVec2(-1, 25)) then
			...
			if imgui.BeginPopupModal(u8("Äîáàâëåíèå êíîïêè"), _, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize + imgui.WindowFlags.NoMove) then
				...
				if retpor_text_edit then
					if imgui.Button(u8("Èçìåíèòü"), imgui.ImVec2(400, 20)) then
						if inputreportaddname.v == "" or inputreportaddname.v == nil or inputreportaddotvet.v == nil or inputreportaddotvet.v == "" then
							...
							sampAddChatMessage("Âû óñïåøíî îòðåäàêòèðîâàëè êíîïêó \"" .. u8:decode(inputreportaddname.v) .. "\"", 15352900)
				...
				elseif imgui.Button(u8("Äîáàâèòü"), imgui.ImVec2(400, 20)) then
					if inputreportaddname.v == "" or inputreportaddname.v == nil or inputreportaddotvet.v == nil or inputreportaddotvet.v == "" then
						...
						inicfg.save(cfg, "AdminToolsKing\\AdminTools.ini")
						sampAddChatMessage("Âû óñïåøíî äîáàâèëè êíîïêó \"" .. u8:decode(inputreportaddname.v) .. "\"", 15352900)
		...
		if menusel.v == 13 then
			...
			imgui.PopFont()
			imgui.PushFont(fs80)
			imgui.SetCursorPosX((imgui.GetWindowWidth() - imgui.CalcTextSize('IKONA CODER').x) / 2)
			imgui.TextColored(imgui.ImVec4(rainbowtext(4)), 'IKONA CODER')
			imgui.PopFont()
			imgui.PushFont(fs16)
			imgui.CenterTextColoredRGB('Ðåàëèçîâàíî äëÿ ïðîåêòà:')
			imgui.SetCursorPosX((imgui.GetWindowWidth() - imgui.CalcTextSize('ARIZONA RP').x) / 2 - 65)
			imgui.TextColored(imgui.ImVec4(rainbowtext(4)), 'ARIZONA RP | vk.com/ikonacoder')
			imgui.PopFont()
			imgui.PushFont(fs16)
		...
		if menusel.v == 6 then
			...
			if imgui.ToggleButton(u8'Âêëþ÷èòü ÷åêåð', imgui.ImBool(cfg.main.adminsChecker)) then 
			...
			if imgui.ToggleButton(u8'Îòîáðàæàòü AFK â ÷åêåðå', imgui.ImBool(cfg.main.adminsafkinchecker)) then 
			...
			if imgui.ColorEdit3(u8'Ñòàíäàðòíûé öâåò íèêîâ', adminsstockcolorchecker, imgui.ColorEditFlags.NoInputs + imgui.ColorEditFlags.AlphaBar) then
	...
	if AmemberWindow.v then
		...
		imgui.SetNextWindowPos(imgui.ImVec2((sw / 2), sh / 2), imgui.Cond.FirstUseEver, imgui.ImVec2(0.5, 0.5))
		imgui.Begin(u8"Âûáåðèòå îðãàíèçàöèþ", AmemberWindow, imgui.WindowFlags.NoResize + imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoSavedSettings)
	...
	if TeleportWindow.v then
		...
		if imgui.Button(u8'ÒÏ ïî ìåòêå', imgui.ImVec2(-1, 25)) then
			...
			if not _ then 
				sampAddChatMessage(tag.."Ìåòêà äëÿ òåëåïîðòà {E78284}íå íàéäåíà{84A6E7}.", -1)  else setCharCoordinates(PLAYER_PED, x, y, z) 
			end
		end
	...
	if CaptchaWindow.v then
		...
		imgui.Begin(fa.ICON_FA_COGS..u8' Íàñòðîéêè | Êàï÷à', CaptchaWindow, imgui.WindowFlags.NoResize + imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoSavedSettings)
...
imgui.BeginChild("qeuqewiqweiqwe5", imgui.ImVec2(-1, 60), true)
		...
		imgui.CenterText(u8('/vr Âíèìàíèå! Êàï÷à: "'..u8:decode(cfg.main.kapchas))..u8('". Ïðèç: '..u8:decode(cfg.main.present))..'!')
		else
		imgui.CenterText(u8('/ao Âíèìàíèå! Êàï÷à: "'..u8:decode(cfg.main.kapchas))..u8('". Ïðèç: '..u8:decode(cfg.main.present))..'!')
		end
imgui.EndChild()
if imgui.Button(u8'Îòïðàâèòü êàï÷ó', imgui.ImVec2(-1, 25)) then
if when_captcha.v == 2 then
sampSendChat('/vr Âíèìàíèå! Êàï÷à: "'..u8:decode(cfg.main.kapchas)..'". Ïðèç: '..u8:decode(cfg.main.present)..'!')
else
sampSendChat('/ao Âíèìàíèå! Êàï÷à: "'..u8:decode(cfg.main.kapchas)..'". Ïðèç: '..u8:decode(cfg.main.present)..'!')
end
end
imgui.End()
	end
	...
	imgui.SetNextWindowSize(imgui.ImVec2(480, 270), imgui.Cond.FirstUseEver)
		imgui.Begin(u8'Òàáëèöà öâåòîâ', ColorsWindow, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize + imgui.WindowFlags.NoSavedSettings + imgui.WindowFlags.AlwaysAutoResize)
...
if PunishWindow.v and cfg.main.menurecon then
	...
	imgui.Begin(u8("Áûñòðûå íàêàçàíèÿ"), _, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize + imgui.WindowFlags.NoMove + imgui.WindowFlags.NoBringToFrontOnFocus + imgui.WindowFlags.NoSavedSettings)
...
if imgui.Button(u8'Óêàçàòü ñâîþ ïðè÷èíó', imgui.ImVec2(-1, 20)) then
   sampSetChatInputEnabled(true)
   sampAddChatMessage(tag..'Íà Arizona RP äàííàÿ ôóíêöèÿ íå ðàáîòàåò!', -1)
   imgui.CloseCurrentPopup()
end
if imgui.Button(u8'Çàêðûòü', imgui.ImVec2(140, 20)) then
imgui.CloseCurrentPopup()
			...
			imgui.BeginChild('##ChangeLog', imgui.ImVec2(-1, imgui.GetWindowHeight() - 105), true)
				for count = 0, #changelog do
					...
					if #patches > 0 then
						...
						imgui.Hint('show_patches:'..count, u8('Êëèêíèòå, ÷òî áû '..(changelog[count]['patches']['show'] and 'ñâåðíóòü' or 'ðàçâåðíóòü')))
						if imgui.IsItemClicked() then 
...
if ReportWindow.v then 
		...
		imgui.Begin(u8'Æàëîáà/Âîïðîñ', ReportWindow, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize + imgui.WindowFlags.NoMove + imgui.WindowFlags.NoSavedSettings + imgui.WindowFlags.AlwaysAutoResize)
			...
			imgui.Hint('helpcur', u8'Åñëè ó âàñ çàáàãàëñÿ êóðñîð, òî ïîôèêñèòü ïðîáëåìó\nìîæíî ïåðåâåäÿ ðàñêëàäêó íà àíãë. ÿç., ïîñëå îòêðîéòå ÷àò,\näàëåå çàêðîéòå åãî, è ïîëå ââîäà ñíîâà ñòàíåò äîñòóïíî!')
			imgui.Separator()
			imgui.Text(u8:encode(report_text))
			imgui.Separator()     
			local otvet = text_buffer_rep.v
			if len(text_buffer_rep.v) > 55 then 
			imgui.CenterTextColoredRGB('{fe4e4e} Îòâåò äîëæåí ñîäåðæàòü íå áîëåå 70 ñèìâîëîâ ['..len(text_buffer_rep.v)..'/70]') 
			end
			...
			if imgui.Button(fa.ICON_FA_ROBOT..u8' Ñëåæó çà íàðóø', imgui.ImVec2(145, 20)) then
				if report_text:match("(%d+)") ~= nil then  
					if sampIsPlayerConnected(report_text:gmatch("(%d+)")) then 
						...
						sampAddChatMessage("{808080}[Ñëåæêà] Âû íå ìîæåòå íà÷àòü ñëåäèòü çà íàðóøèòåëåì,ò.ê íå óêàçàí àéäè â ðåïîðòå èëè èãðîê îôôëàéí.",-1) 
	  				end 
				end 
			end
			imgui.SameLine()
			if imgui.Button(fa.ICON_FA_PEOPLE_CARRY..u8' Ïîìî÷ü àâòîðó', imgui.ImVec2(145, 20)) then
				...
				ReportWindow.v = false
				sampSendChat('/re ' .. report_id) 
				sampAddChatMessage("[Àâòîìàòè÷åñêàÿ ñëåæêà] Âû óøëè â ñëåæêó çà " .. nick_rep .. "[" .. report_id .. "]", 14628149) 
			end
			imgui.SameLine()
...
ReportWindow.v = false
	...
	--imgui.PopStyleColor()
		...
		if imgui.Button(u8' Äà',imgui.ImVec2(btns_sizeX,20)) then sampSendDialogResponse(6370, 1, _, '>> Skipped >>') ReportWindow.v = false imgui.CloseCurrentPopup()   end
		imgui.SameLine()
	...
	if myOnline.v then 
		...
		imgui.Begin(u8'#WeekOnline', _, imgui.WindowFlags.NoResize + imgui.WindowFlags.NoCollapse + imgui.WindowFlags.ShowBorders + imgui.WindowFlags.NoTitleBar + imgui.WindowFlags.AlwaysAutoResize)
			...
			imgui.PushFont(fsClock) imgui.CenterTextColoredRGB('Îíëàéí çà íåäåëþ') imgui.PopFont()
...
if not result then
sampAddChatMessage(tag.."Èãðîê íå â ñåòè {E78284}& {84A6E7}äàííûé èãðîê àäìèíèñòðàòîð. Âîçìîæíî Âû îøèáëèñü {E78284}ID'îì", -1)
else
...
sampSendChat('/pm '..id..' 0 '..u8:decode(cfg.main.infopunish))
sampSendChat('/pm '..id..' 0 '..u8:decode(cfg.main.infopunish1))
sampSendChat('/'..cfg.main.typeoskadm..' '..nick..', '..cfg.main.oskadm.. ' îñêîðáëåíèå àäìè[í]èñòðàöèè')
end
end
end
...
lua_thread.create(function()
sampSendChat('/ao [Ñïàâí Òðàíñïîðòà] Óâàæàåìûå èãðîêè, ÷åðåç '..arg..' ñåêóíä ïðîèçîéäåò ðåñïàâí âñåãî òðàíñïîðòà.')
...
if not _ then 
sampAddChatMessage(tag.."Ìàðêåð äëÿ òåëåïîðòà {E78284}íå íàéäåí{84A6E7}.", -1) else setCharCoordinates(PLAYER_PED, x, y, z) end
end
...
return false
	...
	if dialogTitle:find("Àäìèíèñòðàöèÿ îíëàéí") and cfg.main.adminsChecker then
...
{BFBBBA}Îñíîâíàÿ ñòàòèñòèêà
{00BEFC}Dialog text:{FFFFFF}
Òåêóùåå ñîñòîÿíèå ñ÷åòà: 		8251594 AZ-Coins

{FFFFFF}Èìÿ: {B83434}[Katsu_Evilmane] 
{FFFFFF}Ïîë: {B83434}[Ìóæ÷èíà] 
{FFFFFF}Çäîðîâüå: {B83434}[100/500]
{FFFFFF}Óðîâåíü: {B83434}[10] 
{FFFFFF}Óâàæåíèå: {B83434}[6/14] 
{FFFFFF}Íàëè÷íûå äåíüãè (SA$): {B83434}[$99883699]
{FFFFFF}Íàëè÷íûå äåíüãè (VC$): {B83434}[$0]
{FFFFFF}Åâðî: {B83434}[0] 
{FFFFFF}BTC: {B83434}[212]
{FFFFFF}Íîìåð òåëåôîíà: {B83434}[0] 
{FFFFFF}Äåíüãè â áàíêå: {B83434}[0 BTC]
{FFFFFF}Äåíüãè íà äåïîçèòå: {B83434}[$0]
{FFFFFF}Ðàáîòà: {B83434}[None]
{FFFFFF}Îðãàíèçàöèÿ: {B83434}[Ballas]
{FFFFFF}Äîëæíîñòü: {B83434}Federal Block(7)

{FFFFFF}Óðîâåíü ðîçûñêà: {B83434}[0] 
{FFFFFF}Çàêîíîïîñëóøíîñòü: {B83434}19/100{FFFFFF}

Çàùèòà: {B83434}[-0 óðîíà]{FFFFFF}
Ðåãåíåðàöèÿ: {B83434}[0 HP â ìèí.]{FFFFFF}
Óðîí: {B83434}[+0 óðîíà]{FFFFFF}
Óäà÷à: {B83434}[øàíñ 0 êðèò.óðîíà]{FFFFFF}
Ìàêñ. HP: {B83434}[+0 ìàêñ. HP]{FFFFFF}
Ìàêñ. Áðîíè: {B83434}[+0 ìàêñ. Áðîíè]{FFFFFF}
Øàíñ îãëóøåíèÿ: {B83434}[+0]{FFFFFF}
Øàíñ îãëóøåíèÿ (îãëóøàþùèé ïëîä): {FF6347}Íåàêòèâåí

{FFFFFF}Ïðåäóïðåæäåíèÿ: {B83434}[0]
{FFFFFF}Íàðêîçàâèñèìîñòü: {B83434}0 	{529020}[Íåò çàâèñèìîñòè]
{FFFFFF}Áàíêîâñêàÿ êàðòà: {B83434}[Íå èìååòñÿ]
{FFFFFF}Ñòàòóñ: {B83434}[PREMIUM]
{FFFFFF}Ñåìüÿ: {B83434}Íåò
{FFFFFF}Âîçìîæíîñòü âëàäåòü 5-þ áèçíåñàìè: {B83434}[ÍÅÒ]{FFFFFF}
{FFFFFF}Âîçìîæíîñòü âëàäåòü 4-ÿ äîìàìè: {B83434}[ÍÅÒ]{FFFFFF}
{FFFFFF}Âîçìîæíîñòü âëàäåòü 2-ÿ ôåðìàìè: {B83434}[ÍÅÒ]{FFFFFF}
{FFFFFF}Âîçìîæíîñòü âëàäåòü 2-ÿ ñêëàäñêèìè ïîìåùåíèÿìè: {B83434}[ÍÅÒ]{FFFFFF}

{AFAFAF}Mercedes-Benz S600(896)
Ñòàòóñ áëîêèðîâêè: [Îòêðûò] 
Ñèãíàëèçàöèÿ: [Âûêëþ÷åíà] 
...
Ñèãíàëèçàöèÿ: [Âûêëþ÷åíà] 
	...
	if id == 235 then
		--if dialogText:find("Îðãàíèçàöèÿ: {B83434}%[(.+)%]\n") then
		--	org = dialogText:match("Îðãàíèçàöèÿ: {B83434}%[(.+)%]\n")
		--	rank = 0
		--	sampAddChatMessage('org = '..org, -1)
		--end
		--if dialogText:find("{FFFFFF}Îðãàíèçàöèÿ: {B83434}%[(.*)%]\n{FFFFFF}Äîëæíîñòü: {B83434}(.*)%((%d+)%)\n") then
		--	org, namerank, rank = dialogText:match("{FFFFFF}Îðãàíèçàöèÿ: {B83434}%[(.*)%]\n{FFFFFF}Äîëæíîñòü: {B83434}(.*)%((%d+)%)\n")
		--	sampAddChatMessage('org rank'..org..rank, -1)
		--end
		--if dialogText:find("\n{......}Åâðî: {B83434}%[(%d+)%]\n{......}BTC: {B83434}%[(%d+)%]\n") then
		--	euro, btc = dialogText:match("{......}Åâðî: {B83434}%[(%d+)%]\n{......}BTC: {B83434}%[(%d+)%]\n")
		--end
		--if dialogText:find("Ñòàòóñ: {......}%[(.+)%]\n") then
		--	vipstatus = dialogText:match("Ñòàòóñ: {......}%[(.+)%]\n")
		--end
		--if dialogText:find("{FFFFFF}Ïðåäóïðåæäåíèÿ: {B83434}%[(.*)%]\n") then
		--	warns = dialogText:match("{FFFFFF}Ïðåäóïðåæäåíèÿ: {B83434}%[(.*)%]\n")
		--end
		--org = "Çàãðóçêà"
		--rank = 0
		--euro = "Çàãðóçêà"
		...
		bankbtc = nil
		for line in dialogText:gsub('{......}',''):gmatch('([^\n\r]+)') do
			if line:find("Îðãàíèçàöèÿ: %[(.*)%]") then
				org = line:match("Îðãàíèçàöèÿ: %[(.*)%]")
			end
			if line:find("Äîëæíîñòü: (.*)%((%d+)%)") then
				namerank, rank = line:match("Äîëæíîñòü: (.*)%((%d+)%)")
			end
			if line:find("Åâðî: %[(%d+)%]") then
				euro = line:match("Åâðî: %[(%d+)%]")
			end
			if line:find("BTC: %[(%d+)%]") then
				btc = line:match("BTC: %[(%d+)%]")
			end
			if line:find("Äåíüãè â áàíêå: %[(%d+) BTC%]") then
				bankbtc = line:match("Äåíüãè â áàíêå: %[(%d+) BTC%]")
			end
			if line:find("Ñòàòóñ: %[(.+)%]") then
				vipstatus = line:match("Ñòàòóñ: %[(.+)%]")
			end
			if line:find("Ïðåäóïðåæäåíèÿ: %[(.*)%]") then
				warns = line:match("Ïðåäóïðåæäåíèÿ: %[(.*)%]")
			end	
			if line:find("Çàùèòà: %[%-(%d+) óðîíà%]") then
				protect = line:match("Çàùèòà: %[%-(%d+) óðîíà%]")
			end	
			if line:find("Ðåãåíåðàöèÿ: %[(%d+) HP â ìèí.%]") then
				regen = line:match("Ðåãåíåðàöèÿ: %[(%d+) HP â ìèí.%]")
			end	
			if line:find("Óðîí: %[%+(%d+) óðîíà%]") then
				damag = line:match("Óðîí: %[%+(%d+) óðîíà%]")
			end	
...
idspec, nickspec, lvlspec, uidspec, pl, statespec = text:match("%[(%d+)%] (.*) | Óðîâåíü: (%d+) | UID: (%d+) | packetloss: (.*) %((.*)%)")
...
local f = io.open(getGameDirectory().."//moonloader//AdminToolsKing//Ëîãè//UnjailsLogs.txt", 'a')
f:write('['..os.date("%d.%m.%Y %X", os.time())..'] Àäìèíèñòðàòîð '..adminnick..' âûïóñòèë èãðîêà '..playernick..'. Ïðè÷èíà: '..reason..'\n')
f:close()
end
...
local f = io.open(getGameDirectory().."//moonloader//AdminToolsKing//Ëîãè//UnmutesLogs.txt", 'a')
f:write('['..os.date("%d.%m.%Y %X", os.time())..'] Àäìèíèñòðàòîð '..adminnick..' ñíÿë çàãëóøêó ñ èãðîêà '..playernick..'. Ïðè÷èíà: '..reason..'\n')
f:close()
end
...
local f = io.open(getGameDirectory().."//moonloader//AdminToolsKing//Ëîãè//UnbanLogs.txt", 'a')
f:write('['..os.date("%d.%m.%Y %X", os.time())..'] Àäìèíèñòðàòîð '..adminnick..' ðàçáàíèë èãðîêà '..playernick..'. Ïðè÷èíà: '..reason..'\n')
f:close()
end

if text:find("%[A%] (.*)%[(%d+)%] ðàçáëîêèðîâàë IP: (.*)") and cfg.main.logunbanip then
adminnick, adminid, unbanip = text:match("%[A%] (.*)%[(%d+)%] ðàçáëîêèðîâàë IP: (.*)")
local f = io.open(getGameDirectory().."//moonloader//AdminToolsKing//Ëîãè//UnbanipLogs.txt", 'a')
f:write('['..os.date("%d.%m.%Y %X", os.time())..'] Àäìèíèñòðàòîð '..adminnick..' ðàçáëîêèðîâàë IP: '..unbanip..'\n')
f:close()
end

if text:find("Àäìèíèñòðàòîð (.*) çàáàíèë â îôôëàéíå èãðîêà (.*)%. Ïðè÷èíà: (.*)") and cfg.main.logbanoff then
adminnick, playernick, reason = text:match("Àäìèíèñòðàòîð (.*) çàáàíèë â îôôëàéíå èãðîêà (.*)%. Ïðè÷èíà: (.*)")
local f = io.open(getGameDirectory().."//moonloader//AdminToolsKing//Ëîãè//BanoffLogs.txt", 'a')
f:write('['..os.date("%d.%m.%Y %X", os.time())..'] Àäìèíèñòðàòîð '..adminnick..' çàáëîêèðîâàë â îôôëàéíå èãðîêà '..playernick..'. Ïðè÷èíà: '..reason..'\n')
f:close()
end

if text:find("Àäìèíèñòðàòîð (.*) óñòàíîâèë â îôôëàéíå (%d+) ìèíóò òþðüìû èãðîêó (.*)%. Ïðè÷èíà: (.*)") and cfg.main.logjailoff then
adminnick, srok, playernick, reason = text:match("Àäìèíèñòðàòîð (.*) óñòàíîâèë â îôôëàéíå (%d+) ìèíóò òþðüìû èãðîêó (.*)%. Ïðè÷èíà: (.*)")
local f = io.open(getGameDirectory().."//moonloader//AdminToolsKing//Ëîãè//JailoffLogs.txt", 'a')
f:write('['..os.date("%d.%m.%Y %X", os.time())..'] Àäìèíèñòðàòîð '..adminnick..' óñòàíîâèë â îôôëàéíå '..srok..' ìèíóò òþðüìû èãðîêó '..playernick..'. Ïðè÷èíà: '..reason..'\n')
f:close()
end

if text:find("Àäìèíèñòðàòîð (.*) óñòàíîâèë â îôôëàéíå (%d+) ìèíóò ìîë÷àíêè èãðîêó (.*)%. Ïðè÷èíà: (.*)") and cfg.main.logmuteoff then
adminnick, srok, playernick, reason = text:match("Àäìèíèñòðàòîð (.*) óñòàíîâèë â îôôëàéíå (%d+) ìèíóò ìîë÷àíêè èãðîêó (.*)%. Ïðè÷èíà: (.*)")
local f = io.open(getGameDirectory().."//moonloader//AdminToolsKing//Ëîãè//MuteoffLogs.txt", 'a')
f:write('['..os.date("%d.%m.%Y %X", os.time())..'] Àäìèíèñòðàòîð '..adminnick..' óñòàíîâèë â îôôëàéíå '..srok..' ìèíóò ìîë÷àíêè èãðîêó '..playernick..'. Ïðè÷èíà: '..reason..'\n')
f:close()
end
...
local f = io.open(getGameDirectory().."//moonloader//AdminToolsKing//Ëîãè//BanipoffLogs.txt", 'a')
f:write('['..os.date("%d.%m.%Y %X", os.time())..'] Àäìèíèñòðàòîð '..adminnick..' çàáàíèë IP '..banipoffip..'. Ïðè÷èíà: '..reason..'\n')
f:close()
end
end
if text:find("Ïðèâåòñòâóåì íîâîãî èãðîêà íàøåãî ñåðâåðà: {FF9900}(.*) {FFFFFF}%(ID: (%d+)%)  {cccccc}IP: (%d+)%.(%d+)%.(%d+)%.(%d+)") then
regnick, regid, ip1, ip2, ip3, ip4 = text:match("Ïðèâåòñòâóåì íîâîãî èãðîêà íàøåãî ñåðâåðà: {FF9900}(.*) {FFFFFF}%(ID: (%d+)%)  {cccccc}IP: (%d+)%.(%d+)%.(%d+)%.(%d+)")
if cfg.main.regChecker then
...
sampAddChatMessage('Ïðèâåòñòâóåì íîâîãî èãðîêà íàøåãî ñåðâåðà: {FF9900}'..regnick..' {ffffff}(ID: '..regid..') {cccccc}IP: '..ip1..'.xx.xx.'..ip4, -1)
return false
end
end
if text:find("Èãðîê (.*)%[(%d+)%] %[IP: (%d+).(%d+).(%d+).(%d+)%] ïåðåâåë èãðîêó (.*)%[(%d+)%] %[IP: (%d+).(%d+).(%d+).(%d+)%] íà ñ÷åò $(.*) %[$(.*)%]") then
nickplayer, idplayer, ip1, ip2, ip3, ip4, nickget, idget, ip5, ip6, ip7, ip8, summ, allmoney = text:match("Èãðîê (.*)%[(%d+)%] %[IP: (%d+).(%d+).(%d+).(%d+)%] ïåðåâåë èãðîêó (.*)%[(%d+)%] %[IP: (%d+).(%d+).(%d+).(%d+)%] íà ñ÷åò $(.*) %[$(.*)%]")
if cfg.main.removeip then
sampAddChatMessage('{BA2B29}Èãðîê '..nickplayer..'['..idplayer..'] [IP: '..ip1..'.xx.xx.'..ip4..'] ïåðåâåë èãðîêó '..nickget..'['..idget..'] [IP: '..ip5..'.xx.xx.'..ip8..'] íà ñ÷åò $'..summ..' [$'..allmoney..']', -1)
return false
end
end
...
nickadm, idadm, ip1, ip2, ip3, ip4, ip5, ip6, ip7, ip8 = text:match("Èãðîê/àäìèí (.*)%[(%d+)%] çàøåë %[reg ip:(%d+).(%d+).(%d+).(%d+)%] %-%- %[norm ip: (%d+).(%d+).(%d+).(%d+)%]")
if cfg.main.removeip then
sampAddChatMessage('{BA2B29}Ëèäåð/àäìèíèñòðàòîð '..nickadm..'['..idadm..'] çàøåë íà ñåðâåð. [REG IP: '..ip1..'.xx.xx.'..ip4..'] - [LAST IP: '..ip5..'.xx.xx.'..ip7..']', -1)
return false
end
end
if text:find("Niñk %[(.*)%]  R%-IP %[(.*)%]  IP | A%-IP %[{FF6A78}(.*) | (.*){6ab1ff}%]") then
nickplayer, regip, lastip, xzip = text:match("Niñk %[(.*)%]  R%-IP %[(.*)%]  IP | A%-IP %[{FF6A78}(.*) | (.*){6ab1ff}%]")
if cfg.main.regi then
					...
					if rdata[i]["status"] == "success" then
						...
						if style_reg.v == 1 then
							if style_admin.v == 1 then
								if cmd == 'getip' then
									if cfg.main.removeip then
										sampSendChat('/A [Tools] Ïðîâåðÿþ IP: ñêðûòî | ¹'..i)
									else
										sampSendChat('/A [Tools] Ïðîâåðÿþ IP: '..rdata[i]["query"]..' | ¹'..i)
									end
									sampSendChat('/a ['..i..' IP] Ñòðàíà: '..rdata[i]["country"]..' | Ãîðîä: '..rdata[i]["city"]..' | Ïðîâàéäåð: '..rdata[i]["isp"])
									sampSendChat('/a ['..i..' IP] Ðàññòîÿíèå îò ðåãèñòðàöèîííîãî: '..distances..'êì')
									cmd = "NULL"
								end
							else 
									if cfg.main.removeip then
								...
								if cfg.main.removeip then
									...
									sampAddChatMessage(tag..'Ïðîâåðÿþ IP: {E78284}'..rdata[i]["query"]..' {84A6E7}| {E78284}¹'..i, -1)
								end
								sampAddChatMessage(tag..'['..i..' IP] Ñòðàíà: '..rdata[i]["country"]..' | Ãîðîä: '..rdata[i]["city"]..' | Ïðîâàéäåð: '..rdata[i]["isp"], -1)
								sampAddChatMessage(tag..'['..i..' IP] Ðàññòîÿíèå îò ðåãèñòðàöèîííîãî: '..distances..'êì', -1)
						end
			   end
				end
			end,
	...
	if cfg.main.removeip then
		sampAddChatMessage(tag..'Èãðîâîé íèêíåéì: {E78284}'..nickplayer..'{84A6E7}. REG-IP: {E78284}ñêðûòî {84A6E7}| LAST-IP: {E78284}ñêðûòî', -1)
		return false
	end
end
end
...
if text:find("Èñïîëüçóéòå: /tpcor %[x%] %[y%] %[z%]") then
sampAddChatMessage('{E78284}[AdminTools] {FFFFFF}Èñïîëüçóéòå: /tpc [x] [y] [z]', -1)
return false
end
		...
		if text:match('%[.*%] (%w+_?%w+)%[(%d+)%]%: /'..v..'%s') and cfg.main.trueform then
			...
			if not isGamePaused() then
				...
				or cmd == "setarmour" and cfg.formssettings.autoforma_setarmour or cmd == "unmuteoff" and cfg.formssettings.autoforma_unmuteoff then
						lua_thread.create(function()
							sampAddChatMessage("[AdminTools] {"..cfg.main.twocolorforms.."}Ôîðìà {"..cfg.main.colorforms.. "}/"..cmd.." "..paramssss.." {"..cfg.main.twocolorforms.."} ïðèíÿòà àâòîìàòè÷åñêè!", "0xFF"..cfg.main.colorforms)
							local f = io.open(getGameDirectory().."//moonloader//AdminToolsKing//AcceptForms.txt", 'a')
							f:write('['..os.date("%d.%m.%Y %X", os.time())..'] Ôîðìà: /'..cmd..' '..paramssss..' | Îòïðàâèòåëü: '..admin_nick..' | Òèï ïðèíÿòèÿ ôîðìû: àâòîìàòè÷åñêè\n')
							f:close()
							cfg.main.formsday = cfg.main.formsday + 1
							cfg.main.formsvse = cfg.main.formsvse + 1
							sampSendChat("/"..cmd.." "..paramssss)
		...
		if text:match("%[.*%] (%w+_?%w+)%[(%d+)%]%: /"..v.."%s") and cfg.main.trueform then
			...
			admin_nick, admin_id, other = text:match("%[.+%] (%w+_?%w+)%[(%d+)%]%: /"..v.."%s(.*)")
				...
				or cmd == "setarmour" and cfg.formssettings.forma_setarmour or cmd == "unmuteoff" and cfg.formssettings.forma_unmuteoff then
					...
					if not isGamePaused() then
						sampAddChatMessage("[AdminTools] {"..cfg.main.twocolorforms.."}Íàéäåíà ôîðìà >> {"..cfg.main.colorforms.."}/"..cmd.." "..paramssss.." {"..cfg.main.twocolorforms.."}<< Îòïðàâèòåëü: {"..cfg.main.colorforms.."}".. admin_nick .. "[" .. admin_id .. "]", "0xFF"..cfg.main.colorforms)
						sampAddChatMessage("[AdminTools] {"..cfg.main.twocolorforms.."}Íàæìèòå êëàâèøó >> {"..cfg.main.colorforms.. "}"..klavisha.." {"..cfg.main.twocolorforms.."}<< ÷òîáû ïðèíÿòü ôîðìó!", "0xFF"..cfg.main.colorforms)
						lua_thread.create(function()
						for i = 0, 5 do
						...
						if prikoll == "true" then
							sampAddChatMessage("[AdminTools] {"..cfg.main.twocolorforms.."}Ôîðìà >> {"..cfg.main.colorforms.."}/"..cmd.." "..paramssss.." {"..cfg.main.twocolorforms.."}<< íå ïðèíÿòà!", "0xFF"..cfg.main.colorforms)
...
if not isGamePaused() then	
	...
	if text:find("%[Æàëîáà%] îò (.*)%[(%d+)%]: {ffffff}(.*)%. Óæå {BE2D2D}(%d+){FFFFFF} æàëîá!!!") then
		nickp, idp, sute, kolvo = text:match("%[Æàëîáà%] îò (.*)%[(%d+)%]: {ffffff}(.*)%. Óæå {BE2D2D}(%d+){FFFFFF} æàëîá!!!")
		sampAddChatMessage('[Ðåïîðò] îò '..nickp..'['..idp..']: {'..cfg.main.twocolorreport..'}'..sute..'. Óæå {'..cfg.main.colorreport..'}'..kolvo..' {'..cfg.main.twocolorreport..'}ðåïîðòîâ!', '0xFF'..cfg.main.colorreport)
	...
	if text:find("%[A] (Sasha_Gozhelnikov)%[(%d+)%]: bot check updates") then
		anickname, aid = text:match("%[A] (Sasha_Gozhelnikov)%[(%d+)%]: bot check updates")
				sampSendChat('/a Âåðñèÿ ñêðèïòà: 29.03.2023')
	end

		if text:find("%[A] (Sasha_Gozhelnikov)%[(%d+)%]: bot obnova") then
		anickname, aid = text:match("%[A] (Sasha_Gozhelnikov)%[(%d+)%]: bot obnova")
				sampSendChat('/a íîâàÿ âåðñèÿ ñêðèïòà, óñòàíîâèòü: vk.com/ikonacoder')
	end

	if text:find("(.*) %[(.*)%] êóïèë áèçíåñ ID: (.*) ïî ãîñ. öåíå çà (.*) ms! Êàï÷à: %((%d+) | (%d+)%)") and cfg.autoopra.settingbiz then
		bnickp, bidp, bidbiz, bvremya, bcaptcha1, bcaptcha2 = text:match("(.*) %[(.*)%] êóïèë áèçíåñ ID: (.*) ïî ãîñ. öåíå çà (.*) ms! Êàï÷à: %((%d+) | (%d+)%)")
		nick_cutch = bnickp
		idp_cutch = bidp
		id_cutch = bidbiz
		time_cutch = bvremya
		...
		sampAddChatMessage(tag .. '{B2D8F4}Èñïîëüçóéòå êîìàíäó {F4A8B2}/' .. cmdBusiness.v .. ' {B2D8F4}äëÿ çàïðîñà îïðîâåðæåíèÿ íà áèçíåñ {F4A8B2}¹' .. bidbiz, -1)
		end
		local f = io.open(getGameDirectory().."\\moonloader\\AdminToolsKing\\Ëîãè\\AutoOpraLogs.txt", 'a')
		f:write(string.format('%s | Èãðîê '..bnickp..'  áèçíåñ ID: '..bidbiz..' ïî ãîñ. öåíå çà '..bvremya..' ms! Êàï÷à: '..bcaptcha1..'\n', os.date('%d.%m %H:%M', os.time())))
		f:close()
	...
	local admin, victim, tj = text:match("Àäìèíèñòðàòîð ([a-zA-Z_]+).* ïîñàäèë èãðîêà ([a-zA-Z_]+).* â äåìîðãàí íà %d+ ìèíóò")
	...
	local adminoff, tjoff, victimoff = text:match("Àäìèíèñòðàòîð ([A-z_]+) óñòàíîâèë â îôôëàéíå (%d+) ìèíóò äåìîðãàíà èãðîêó ([0-9A-z_]+)%.") 
	for k, box in pairs(tLog) do
	...
	local adminunjoff, victimunjoff = text:match("Àäìèíèñòðàòîð ([a-zA-Z_]+) âûïóñòèë â îôôëàéíå ñ äåìîðãàíà ([a-zA-Z_]+)")
	...
	if text:find("(.*) %[(.*)%] êóïèë äîì ID: (.*) ïî ãîñ. öåíå çà (.*) ms! Êàï÷à: %((%d+) | (%d+)%)") and cfg.autoopra.settinghouse then
		nickp, idp, idhouse, vremya, captcha1, captcha2 = text:match("(.*) %[(.*)%] êóïèë äîì ID: (.*) ïî ãîñ. öåíå çà (.*) ms! Êàï÷à: %((%d+) | (%d+)%)")
		nick_cutch = nickp
		idp_cutch = idp
		id_cutch = idhouse
		time_cutch = vremya
		...
		sampAddChatMessage(tag..'Èñïîëüçóéòå êîìàíäó {E78284}/'..cmdHouse.v..' {84A6E7}äëÿ çàïðîñà îïðîâåðæåíèÿ íà äîì {E78284}¹'..idhouse, -1)
		end
		local f = io.open(getGameDirectory().."\\moonloader\\AdminToolsKing\\Ëîãè\\AutoOpraLogs.txt", 'a')
		f:write(string.format('%s | Èãðîê '..nickp..' êóïèë äîì ID: '..idhouse..' ïî ãîñ. öåíå çà '..vremya..' ms! Êàï÷à: '..captcha1..'\n', os.date('%d.%m %H:%M', os.time())))
		f:close()
	...
	if text:find("%[MakeAdmin%] .*%[%d+%] óñòàíîâèë (%d+) %[.*%] óðîâåíü àäìèíèñòðàòîðà èãðîêó (.*) %(áûë.*") then
		lvladmin, nickchela = text:match("%[MakeAdmin%] .*%[%d+%] óñòàíîâèë (%d+) %[.*%] óðîâåíü àäìèíèñòðàòîðà èãðîêó (.*) %(áûë.*")
...
local _, myid = sampGetPlayerIdByCharHandle(playerPed)
	...
	if text:find("Àäìèíèñòðàòîð "..mynick.."%[(%d+)%] îòâåòèë èãðîêó (.*)%[(%d+)%]:{ffffff} (.*)") and cfg.main.inforeport then
		local _, myid = sampGetPlayerIdByCharHandle(playerPed)
		mynick = sampGetPlayerNickname(myid)
		myidplayer, nickplayer, idplayer, textplayer = text:match("Àäìèíèñòðàòîð "..mynick.."%[(%d+)%] îòâåòèë èãðîêó (.*)%[(%d+)%]:{ffffff} (.*)")
		...
		if text:find("Àäìèíèñòðàòîð (.*)%[(%d+)%] çàáàíèë èãðîêà (.*)%[(%d+)%] íà (%d+) äíåé%. Ïðè÷èíà: (.*)") then
			nickadmin, idadmin, nickp, idp, days, reason = text:match("Àäìèíèñòðàòîð (.*)%[(%d+)%] çàáàíèë èãðîêà (.*)%[(%d+)%] íà (%d+) äíåé%. Ïðè÷èíà: (.*)")
		end
	...
	if text:find("Àäìèíèñòðàòîð (.*)%[(%d+)%] îòâåòèë èãðîêó (.*)%[(%d+)%]:{ffffff} (.*)") then
		nickadmina, idadmina, nickplayer, idplayer, textadmin = text:match("Àäìèíèñòðàòîð (.*)%[(%d+)%] îòâåòèë èãðîêó (.*)%[(%d+)%]:{ffffff} (.*)")
		sampAddChatMessage('[A] '..nickadmina..'['..idadmina..'] îòâåòèë èãðîêó '..nickplayer..'['..idplayer..']: {ffffff}'..textadmin, 0xFFFAAC58)
		return false
	end
	...
	if textWithoutColor:find("%[Âàæíî%] Õî÷åøü ïîëó÷àòü áîíóñû") then
	if cfg.main.autoadm then
		sampSendChat('/apanel')
		closepanel = true
	end
	...
	--Íà ñåðâåðå åñòü èíâåíòàðü, èñïîëüçóéòå êëàâèøó Y äëÿ ðàáîòû ñ íèì.

	if text:find("%[A%] Âû óñïåøíî àâòîðèçîâàëèñü êàê {......}Àäìèíèñòðàòîð (%d+)ãî óðîâíÿ") and cfg.main.myserver == 3 then
		lvladminki = text:match("%[A%] Âû óñïåøíî àâòîðèçîâàëèñü êàê {......}Àäìèíèñòðàòîð (%d+)ãî óðîâíÿ")
		if cfg.main.geniumlvl then
			cfg.main.mylvladmin = lvladminki
			tttttt.v = lvladminki
		end
		if cfg.main.mylvladmin == 8 then
			sampAddChatMessage('%[A%] Âû óñïåøíî àâòîðèçîâàëèñü êàê {fe4e4e}Âëàäåëåö [LVL: '..cfg.main.mylvladmin..' | FullDostup: '..cfg.main.mylvlfd..']', -1)
		else
			sampAddChatMessage('%[A%] Âû óñïåøíî àâòîðèçîâàëèñü êàê Àäìèíèñòðàòîð '..lvladminki..'ãî óðîâíÿ', -1)	
		end
		lua_thread.create(function()
	...
	if text:find("%[A%] Âû óñïåøíî àâòîðèçîâàëèñü êàê {......}(.*)") and cfg.main.myserver == 1 or cfg.main.myserver == 2 and text:find("%[A%] Âû óñïåøíî àâòîðèçîâàëèñü êàê {......}(.*)") then
		...
		if cfg.main.mylvladmin == 8 then
			sampAddChatMessage('%[A%] Âû óñïåøíî àâòîðèçîâàëèñü êàê {fe4e4e}Íà÷àëüíèê [LVL: '..cfg.main.mylvladmin..' | FullDostup: '..cfg.main.mylvlfd..']', -1)
		else
...
function imgui.GetOpraStatus(count, tInfoJail)
		...
		elseif tInfoJail[1] and tInfoJail[2] and tInfoJail[3] then
			...
			imgui.Hint("status_ajail:"..count, u8("Îïðîâåðæåíèå çàïðîøåíî\nÇàïðîñèë: "..tInfoJail[2].."\nÂðåìÿ: "..os.date("%H:%M:%S", tInfoJail[3])))
		elseif not tInfoJail[1] and tInfoJail[2] and tInfoJail[3] then 
			...
			imgui.Hint("status_ajail:"..count, u8("Îïðîâåðæåíèå ïðîâåðåíî\nÂûïóñòèë: "..tInfoJail[2].."\nÂðåìÿ: "..os.date("%H:%M:%S", tInfoJail[3])))
		else
...
function kotofeevstatsadm()
	...
	if imgui.RadioButton(u8'Îòîáðàæàòü îíëàéí (òåêóùàÿ ñåññèÿ)', Radio['sesOnline']) then Radio['sesOnline'] = not Radio['sesOnline']; cfg.statTimers.sesOnline = Radio['sesOnline'] end
	if imgui.RadioButton(u8'Îòîáðàæàòü îíëàéí çà äåíü', Radio['dayOnline']) then Radio['dayOnline'] = not Radio['dayOnline']; cfg.statTimers.dayOnline = Radio['dayOnline'] end
	if imgui.RadioButton(u8'Îòîáðàæàòü îíëàéí â àôê', Radio['dayAfk']) then Radio['dayAfk'] = not Radio['dayAfk']; cfg.statTimers.dayAfk = Radio['dayAfk'] end
	if imgui.RadioButton(u8'Îòîáðàæàòü îíëàéí çà íåäåëþ (áåç ó÷åòà àôê)', Radio['weekOnline']) then Radio['weekOnline'] = not Radio['weekOnline']; cfg.statTimers.weekOnline = Radio['weekOnline'] end
	if imgui.RadioButton(u8'Îòîáðàæàòü îíëàéí àôê çà íåäåëþ', Radio['weekAfk']) then Radio['weekAfk'] = not Radio['weekAfk']; cfg.statTimers.weekAfk = Radio['weekAfk'] end
	if imgui.RadioButton(u8'Îòîáðàæàòü îáùèé îíëàéí çà íåäåëþ (ñ ó÷åòîì àôê)', Radio['weekFull']) then Radio['weekFull'] = not Radio['weekFull']; cfg.statTimers.weekFull = Radio['weekFull'] end
	if imgui.RadioButton(u8'Îòîáðàæàòü âûäàííûå íàêàçàíèÿ çà äåíü', Radio['nakazaniyaDay']) then Radio['nakazaniyaDay'] = not Radio['nakazaniyaDay']; cfg.statTimers.nakazaniyaDay = Radio['nakazaniyaDay'] end
	if imgui.RadioButton(u8'Îòîáðàæàòü âûäàííûå íàêàçàíèÿ çà âñå âðåìÿ', Radio['nakazaniyaVse']) then Radio['nakazaniyaVse'] = not Radio['nakazaniyaVse']; cfg.statTimers.nakazaniyaVse = Radio['nakazaniyaVse'] end
	if imgui.RadioButton(u8'Îòîáðàæàòü âûäàííûå ôîðì çà äåíü', Radio['formsDay']) then Radio['formsDay'] = not Radio['formsDay']; cfg.statTimers.formsDay = Radio['formsDay'] end
	if imgui.RadioButton(u8'Îòîáðàæàòü âûäàííûå ôîðì çà âñå âðåìÿ', Radio['formsVse']) then Radio['formsVse'] = not Radio['formsVse']; cfg.statTimers.formsVse = Radio['formsVse'] end
	if imgui.RadioButton(u8'Îòîáðàæàòü ðåïîðòîâ îòâå÷åíî çà äåíü', Radio['reportsDay']) then Radio['reportsDay'] = not Radio['reportsDay']; cfg.statTimers.reportsDay = Radio['reportsDay'] end
	if imgui.RadioButton(u8'Îòîáðàæàòü ðåïîðòîâ îòâå÷åíî çà âñå âðåìÿ', Radio['reportsVse']) then Radio['reportsVse'] = not Radio['reportsVse']; cfg.statTimers.reportsVse = Radio['reportsVse'] end
	if imgui.RadioButton(u8'Îòîáðàæàòü êîë-âî ðåïîðòîâ íà äàííûé ìîìåíò', Radio['reportnow']) then Radio['reportnow'] = not Radio['reportnow']; cfg.statTimers.reportnow = Radio['reportnow'] end
	if imgui.RadioButton(u8'Îòîáðàæàòü òåêóùåå âðåìÿ', Radio['nowTime']) then Radio['nowTime'] = not Radio['nowTime']; cfg.statTimers.nowTime = Radio['nowTime'] end
	imgui.Separator()
	imgui.PushFont(fontsize)
	imgui.CenterText(u8'Ïðåäîñìîòð')
	imgui.PopFont()
...
elseif logBut["selected"] == 5 then
	...
	imgui.SameLine()
	imgui.Text(fa.ICON_FA_QUESTION_CIRCLE..u8'')
	imgui.Hint('bizopra', u8'Ïðè ââîäå êîìàíäû /autoopra áóäåò âêëþ÷åíà àâòî-îïðà íà áèçíåñû (ò.å. åñëè ÷åëîâåê ïîéìàë áèçíåñ - åãî ïîñàäèò â äåìîðãàí)')
	...
	imgui.SameLine()
	imgui.Text(fa.ICON_FA_QUESTION_CIRCLE..u8'')
	imgui.Hint('houseopra', u8'Ïðè ââîäå êîìàíäû /autoopra áóäåò âêëþ÷åíà àâòî-îïðà íà äîìà (ò.å. åñëè ÷åëîâåê ïîéìàë äîì - åãî ïîñàäèò â äåìîðãàí)')
...
if #tLog == 0 then 
	...
	for k, box in pairs(tLog) do
		if box[1] == 1 then
			...
			imgui.SameLine(30)
			imgui.Text(string.format(u8"| Èãðîê %s êóïèë äîì ID: %s ïî ãîñ. öåíå çà %s! Êàï÷à: %s", box[4][1], box[4][2], box[4][3], box[4][4]))
		...
		if box[1] == 3 then
			...
			imgui.SameLine(30)
			imgui.Text(string.format(u8"| Èãðîê %s êóïèë áèçíåñ ID: %s ïî ãîñ. öåíå çà %s! Êàï÷à: %s", box[4][1], box[4][2], box[4][3], box[4][4]))
			imgui.Hint("plInt:"..k,  u8(os.date("%H:%M:%S", box[2]).."\nÏÊÌ - Âçàèìîäåéñòâîâàòü") )
			if imgui.IsItemClicked(1) then contK = k; imgui.OpenPopup("##toolContext") end
			imgui.GetOpraStatus(k, box[5])
		end
	...
	if contK ~= nil and imgui.BeginPopupContextItem("##toolContext") then
		...
		if not tLog[contK][5][1] then
			if imgui.Button(fa.ICON_FA_BED..u8(" Çàïðîñèòü îïðó"..(plOnline and "" or " â îôôëàéíå")), imgui.ImVec2(220, 30)) then 
				...
				sampSendChat(string.format("/jail%s %s %s îïðà [¹%s | %s]", (plOnline and "" or "off"), tLog[contK][4][1], cfg.main.timeopra, tLog[contK][4][2], tLog[contK][4][3]))
				imgui.CloseCurrentPopup()
			end
		else
			if imgui.Button(fa.ICON_FA_CHECK..u8(" Îäîáðèòü"), imgui.ImVec2(107.5, 30)) then 
				sampSendChat(string.format("/unjail%s %s îïðà ïðèíÿòà", (plOnline and "" or "off"), tLog[contK][4][1]))
				imgui.CloseCurrentPopup()
			end
			imgui.SameLine()
			if imgui.Button(fa.ICON_FA_TIMES..u8(" Îòêëîíèòü"), imgui.ImVec2(107.5, 30)) then 
				sampSendChat(string.format("/unjail%s %s îïðà îòêàçàíà", (plOnline and "" or "off"), tLog[contK][4][1]))
...
imgui.TextDisabled(fa.ICON_FA_QUESTION_CIRCLE..u8'')
imgui.PopFont()
imgui.Hint('helplist', u8'Äîïîëíèòåëüíîå îïîâåùåíèå â ÷àò â ñëó÷àå åñëè èãðîê, ïîéìàâøèé\nèìóùåñòâî, íàõîäèëñÿ â áåëîì ñïèñêå')
...
imgui.TextDisabled(fa.ICON_FA_QUESTION_CIRCLE..u8'')
imgui.PopFont()
imgui.Hint('helplis1t', u8'Ñêðèïò áóäåò óäàëÿòü ñåðâåðíîå ñîîáùåíèå î ïîêóïêå èìóùåñòâà èãðîêîì èç áåëîãî ñïèñêà')
imgui.BeginChild("##IngonList", imgui.ImVec2(-1, 375), true)
		...
		else imgui.DisableButton(u8"Äîáàâèòü", imgui.ImVec2(-1, 20)) end
...
sampAddChatMessage('{E78284}[AdminTools] {84A6E7}Âû óñòàíîâèëè íîâóþ òî÷êó ñïàâíà {E78284}['..myCoordinates[1]..' | '..myCoordinates[2]..' | '..myCoordinates[3]..']', -1)
end
...
sampAddChatMessage(tag..'Îòîáðàæåíèå ñîîáùåíèé î ïîäîçðåíèè íà ðåêëàìó '..(cfg.main.deletereklama and '{ff4040}âûêëþ÷åíî{84A6E7}.' or '{00ff00}âêëþ÷åíî{84A6E7}.'), -1)
end
...
imgui.BeginChild("ChildWdsahdhindow1", imgui.ImVec2(-1, -1), true) 
			...
			if imgui.SliderInt("##bulletsMyTime1", elements.int.timeRenderMyBullets, 5, 40) then
				cfg.config.timeRenderMyBullets = elements.int.timeRenderMyBullets.v
				 inicfg.save(cfg, "AdminToolsKing\\AdminTools.ini")
			end imgui.SameLine() imgui.Text(u8"Âðåìÿ çàäåðæêè òðåéñåðà")
			...
			if imgui.SliderInt("##renderWidthLinesTwo1", elements.int.sizeOffMyLine, 1, 10) then
				cfg.config.sizeOffMyLine = elements.int.sizeOffMyLine.v
				 inicfg.save(cfg, "AdminToolsKing\\AdminTools.ini")
			end imgui.SameLine() imgui.Text(u8"Òîëùèíà ëèíèé")
			...
			if imgui.SliderInt('##maxMyBullets1', elements.int.maxLineMyLimit, 10, 300) then
				...
				cfg.config.maxLineMyLimit = elements.int.maxLineMyLimit.v
				 inicfg.save(cfg, "AdminToolsKing\\AdminTools.ini")
			end imgui.SameLine() imgui.Text(u8"Ìàêñèìàëüíîå êîëè÷åñòâî ëèíèé")
			...
			if imgui.SliderInt('##endNumber1s', elements.int.rotationMyPolygonEnd, 2, 10) then
				cfg.config.rotationMyPolygonEnd = elements.int.rotationMyPolygonEnd.v 
				 inicfg.save(cfg, "AdminToolsKing\\AdminTools.ini")
			end imgui.SameLine() imgui.Text(u8"Êîëè÷åñòâî óãëîâ íà îêîí÷àíèÿõ")
			...
			if imgui.SliderInt('##rotationOne1', elements.int.degreeMyPolygonEnd, 0, 360) then
				cfg.config.degreeMyPolygonEnd = elements.int.degreeMyPolygonEnd.v
				 inicfg.save(cfg, "AdminToolsKing\\AdminTools.ini")
			end imgui.SameLine() imgui.Text(u8"Ãðàäóñ ïîâîðîòà îêîí÷àíèÿ")
			...
			if imgui.SliderInt('##sizeTraicerEnd1', elements.int.sizeOffMyPolygonEnd, 1, 10) then
				cfg.config.sizeOffMyPolygonEnd = elements.int.sizeOffMyPolygonEnd.v
				 inicfg.save(cfg, "AdminToolsKing\\AdminTools.ini")
			end  imgui.SameLine() imgui.Text(u8"Ðàçìåð îêîí÷àíèÿ òðåéñåðà")
			...
			if imgui.ColorEdit4("##dinamicObjectMy", dinamicObjectMy, imgui.ColorEditFlags.NoInputs + imgui.ColorEditFlags.AlphaBar) then
				cfg.config.dinamicObjectMy = join_argb(dinamicObjectMy.v[1] * 255, dinamicObjectMy.v[2] * 255, dinamicObjectMy.v[3] * 255, dinamicObjectMy.v[4] * 255)
				 inicfg.save(cfg, "AdminToolsKing\\AdminTools.ini")
			end imgui.SameLine() imgui.Text(u8"Âû ïîïàëè â äèíàìè÷åñêèé îáúåêò")
			if imgui.ColorEdit4("##staticObjectMy", staticObjectMy, imgui.ColorEditFlags.NoInputs + imgui.ColorEditFlags.AlphaBar) then
				cfg.config.staticObjectMy = join_argb(staticObjectMy.v[1] * 255, staticObjectMy.v[2] * 255, staticObjectMy.v[3] * 255, staticObjectMy.v[4] * 255)
				 inicfg.save(cfg, "AdminToolsKing\\AdminTools.ini")
			end imgui.SameLine() imgui.Text(u8"Âû ïîïàëè â ñòàòè÷åñêèé îáúåêò")
			if imgui.ColorEdit4("##pedMy", pedPMy, imgui.ColorEditFlags.NoInputs + imgui.ColorEditFlags.AlphaBar) then
				cfg.config.pedPMy = join_argb(pedPMy.v[1] * 255, pedPMy.v[2] * 255, pedPMy.v[3] * 255, pedPMy.v[4] * 255)
				 inicfg.save(cfg, "AdminToolsKing\\AdminTools.ini")
			end imgui.SameLine() imgui.Text(u8"Âû ïîïàëè â èãðîêà")
			if imgui.ColorEdit4("##carMy", carPMy, imgui.ColorEditFlags.NoInputs + imgui.ColorEditFlags.AlphaBar) then
				cfg.config.carPMy = join_argb(carPMy.v[1] * 255, carPMy.v[2] * 255, carPMy.v[3] * 255, carPMy.v[4] * 255)
				 inicfg.save(cfg, "AdminToolsKing\\AdminTools.ini")
			end imgui.SameLine() imgui.Text(u8"Âû ïîïàëè â ìàøèíó")
			imgui.PopItemWidth()
			imgui.Separator()
			imgui.PushFont(fs16)
		...
		imgui.PopFont()
			...
			if imgui.ColorEdit4("##infoNickId", colorPlayerI, imgui.ColorEditFlags.NoInputs + imgui.ColorEditFlags.AlphaBar) then
				cfg.config.colorPlayerI = join_argb(colorPlayerI.v[1] * 255, colorPlayerI.v[2] * 255, colorPlayerI.v[3] * 255, colorPlayerI.v[4] * 255)
				 inicfg.save(cfg, "AdminToolsKing\\AdminTools.ini")
			end imgui.SameLine() imgui.Text(u8"Óêàæèòå öâåò íèêíåéìà ñòðåëêà")
			...
			if imgui.ColorEdit4("##dinamicObject", dinamicObject, imgui.ColorEditFlags.NoInputs + imgui.ColorEditFlags.AlphaBar) then
				cfg.config.dinamicObject = join_argb(dinamicObject.v[1] * 255, dinamicObject.v[2] * 255, dinamicObject.v[3] * 255, dinamicObject.v[4] * 255)
				 inicfg.save(cfg, "AdminToolsKing\\AdminTools.ini")
			end imgui.SameLine() imgui.Text(u8"Èãðîê ïîïàë â äèíàìè÷åñêèé îáúåêò")
			if imgui.ColorEdit4("##staticObject", staticObject, imgui.ColorEditFlags.NoInputs + imgui.ColorEditFlags.AlphaBar) then
				cfg.config.staticObject = join_argb(staticObject.v[1] * 255, staticObject.v[2] * 255, staticObject.v[3] * 255, staticObject.v[4] * 255)
				 inicfg.save(cfg, "AdminToolsKing\\AdminTools.ini")
			end imgui.SameLine() imgui.Text(u8"Èãðîê ïîïàë â ñòàòè÷åñêèé îáúåêò")
			if imgui.ColorEdit4("##ped", pedP, imgui.ColorEditFlags.NoInputs + imgui.ColorEditFlags.AlphaBar) then
				cfg.config.pedP = join_argb(pedP.v[1] * 255, pedP.v[2] * 255, pedP.v[3] * 255, pedP.v[4] * 255)
				 inicfg.save(cfg, "AdminToolsKing\\AdminTools.ini")
			end imgui.SameLine() imgui.Text(u8"Èãðîê ïîïàë â èãðîêà")
			if imgui.ColorEdit4("##car", carP, imgui.ColorEditFlags.NoInputs + imgui.ColorEditFlags.AlphaBar) then
				cfg.config.carP = join_argb(carP.v[1] * 255, carP.v[2] * 255, carP.v[3] * 255, carP.v[4] * 255)
				 inicfg.save(cfg, "AdminToolsKing\\AdminTools.ini")
			end imgui.SameLine() imgui.Text(u8"Èãðîê ïîïàë â ìàøèíó")
			imgui.PopItemWidth()
			imgui.Separator()
			imgui.EndChild()
end
...
function ajailThread()
	while true do wait(0)
		if isGamePaused() and cfg.autoopra.status then
			...
			cfg.autoopra.status = false
			sampShowDialog(5454, tag..'Ïðåäóïðåæäåíèå', '{488cff}Àâòîìàòè÷åñêèé ðåæèì áûë âûêëþ÷åí èç-çà ñâîðà÷èâàíèÿ èãðû', 'Ïîíÿë', _, 0)
		end
	end
end
...
_, x, y, z = getTargetBlipCoordinatesFixed()
	if not _ then 
	sampAddChatMessage(tag.."Ìåòêà äëÿ òåëåïîðòà {E78284}íå íàéäåíà{84A6E7}.", -1)  else setCharCoordinates(PLAYER_PED, x, y, z) end
...
function regcommands()
	...
	sampRegisterChatCommand('autoopra', function()
		...
		sampAddChatMessage(tag..'Àâòîîïðîâåðæåíèå '..(cfg.autoopra.status and '{E78284}âêëþ÷åíî' or '{E78284}âûêëþ÷åíî'), -1)
	end)
...
function cccc()
	if cfg.main.playersChecker then 
		...
		renderFontDrawTextAlign(fonts.playersChecker, "Èãðîêè îíëàéí: ", cfg.checkerPosition.x, cfg.checkerPosition.y, -1, cfg.main.align)
		for k,v in ipairs(J_.PLAYERS_CHECKER[1]) do 
	...
	if cfg.main.adminsChecker then 
		...
		renderFontDrawTextAlign(fonts456.adminsChecker, "Àäìèíèñòðàöèÿ îíëàéí: ", cfg.adminscheckerPosition.x, cfg.adminscheckerPosition.y, -1, cfg.main.adminsAlign)
		for i = 1,#adminsTable do
...
if not doesDirectoryExist(getGameDirectory()..'//moonloader//AdminToolsKing//Ëîãè//') then 
	createDirectory(getGameDirectory()..'//moonloader//AdminToolsKing//Ëîãè//') 
	...
	if not doesFileExist(getGameDirectory().."//moonloader//AdminToolsKing//groupbinder.txt") then
		...
		f:write(u8'/ao Óâàæàåìûå èãðîêè, õîòèòå óçíàâàòü àêòóàëüíóþ èíôîðìàöèþ ñàìûì ïåðâûì?\n/ao Äëÿ òàêîãî ñëó÷àÿ ó íàñ åñòü ïàáëèê ÂÊ - vk.com/arizona_kingrp, óçíàâàéòå íîâîñòè ñàìûì ïåðâûì!')
		f:close()
		sampAddChatMessage(tag..'Ôàéë äëÿ áèíäåðà "Ïàáëèê VK" óñïåøíî ñîçäàí', -1)
	end
	
	if not doesFileExist(getGameDirectory().."//moonloader//AdminToolsKing//Ëîãè//AdminVipChatLogs.txt") then
		local f = io.open(getGameDirectory().."//moonloader//AdminToolsKing//Ëîãè//AdminVipChatLogs.txt", 'w')
		f:close()
		sampAddChatMessage(tag..'Ôàéë äëÿ çàïèñè ñîîáùåíèé îò àäìèíèñòðàòîðîâ â VIP ÷àòå ñîçäàí.', -1)
	end
	if not doesFileExist(getGameDirectory().."//moonloader//AdminToolsKing//Ëîãè//UnjailsLogs.txt") then
		local f = io.open(getGameDirectory().."//moonloader//AdminToolsKing//Ëîãè//UnjailsLogs.txt", 'w')
		f:close()
		sampAddChatMessage(tag..'Ôàéë äëÿ çàïèñè âûïóñêîâ èç äåìîðãàíà ñîçäàí.', -1)
	end
	if not doesFileExist(getGameDirectory().."//moonloader//AdminToolsKing//Ëîãè//UnmutesLogs.txt") then
		local f = io.open(getGameDirectory().."//moonloader//AdminToolsKing//Ëîãè//UnmutesLogs.txt", 'w')
		f:close()
		sampAddChatMessage(tag..'Ôàéë äëÿ çàïèñè ñíÿòèé ìóòîâ ñîçäàí.', -1)
	end
	if not doesFileExist(getGameDirectory().."//moonloader//AdminToolsKing//Ëîãè//UnbanLogs.txt") then
		local f = io.open(getGameDirectory().."//moonloader//AdminToolsKing//Ëîãè//UnbanLogs.txt", 'w')
		f:close()
		sampAddChatMessage(tag..'Ôàéë äëÿ çàïèñè ðàçáëîêèðîâîê ñîçäàí.', -1)
	end
	if not doesFileExist(getGameDirectory().."//moonloader//AdminToolsKing//Ëîãè//UnbanipLogs.txt") then
		local f = io.open(getGameDirectory().."//moonloader//AdminToolsKing//Ëîãè//UnbanipLogs.txt", 'w')
		f:close()
		sampAddChatMessage(tag..'Ôàéë äëÿ çàïèñè ðàçáëîêèðîâîê IP ñîçäàí.', -1)
	end
	if not doesFileExist(getGameDirectory().."//moonloader//AdminToolsKing//Ëîãè//BanoffLogs.txt") then
		local f = io.open(getGameDirectory().."//moonloader//AdminToolsKing//Ëîãè//BanoffLogs.txt", 'w')
		f:close()
		sampAddChatMessage(tag..'Ôàéë äëÿ çàïèñè áëîêèðîâîê â îôôëàéíå ñîçäàí.', -1)
	end
	if not doesFileExist(getGameDirectory().."//moonloader//AdminToolsKing//Ëîãè//JailoffLogs.txt") then
		local f = io.open(getGameDirectory().."//moonloader//AdminToolsKing//Ëîãè//JailoffLogs.txt", 'w')
		f:close()
		sampAddChatMessage(tag..'Ôàéë äëÿ çàïèñè âûäà÷è äåìîðãàíîâ â îôôëàéíå ñîçäàí.', -1)
	end
	if not doesFileExist(getGameDirectory().."//moonloader//AdminToolsKing//Ëîãè//MuteoffLogs.txt") then
		local f = io.open(getGameDirectory().."//moonloader//AdminToolsKing//Ëîãè//MuteoffLogs.txt", 'w')
		f:close()
		sampAddChatMessage(tag..'Ôàéë äëÿ çàïèñè âûäà÷è ìóòîâ â îôôëàéíå ñîçäàí.', -1)
	end
	if not doesFileExist(getGameDirectory().."//moonloader//AdminToolsKing//Ëîãè//BanipoffLogs.txt") then
		local f = io.open(getGameDirectory().."//moonloader//AdminToolsKing//Ëîãè//BanipoffLogs.txt", 'w')
		f:close()
		sampAddChatMessage(tag..'Ôàéë äëÿ çàïèñè âûäà÷è áëîêèðîâîê IP â îôôëàéíå ñîçäàí.', -1)
	end
	
	if not doesFileExist(getWorkingDirectory().."\\AdminToolsKing\\Ëîãè\\AutoOpraLogs.txt") then
		local f = io.open(getGameDirectory().."//moonloader//AdminToolsKing//Ëîãè//AutoOpraLogs.txt", 'w')
		f:close()
	...
	for _, v in pairs(peds) do
		...
		if result and id ~= sampGetPlayerIdByCharHandle(PLAYER_PED) and distance < tonumber(arg) then
			if id ~= myid then
				...
				if lvl < 15 then
					sampAddChatMessage(tag..'Íèêíåéì: {fe4e4e}'..nick..' ['..id..'] {84A6E7}| Óðîâåíü: {fe4e4e}'..lvl..' [ÏÓÑÒÛØÊÀ]', -1)
				else
					sampAddChatMessage(tag..'Íèêíåéì: {E78284}'..nick..' ['..id..'] {84A6E7}| Óðîâåíü: {E78284}'..lvl, -1)
				end
			end
		end
	end
...
if check_input_available() and isKeysZazhat(fvzaim.v) then 
				...
				local mltÑorner = math.floor(cfg.main.circleRadius / 100) == 0 and 1 or math.floor(cfg.main.circleRadius / 100)
				for i = 1, 2 do renderFigure2D(cx, cy, 25 * mltÑorner, cfg.main.circleRadius + i, (cfg.main.targetState == "player" and PLAYER_COLOR or ACTIVE_COLOR)) end
				renderFontDrawTextCenter(fonts1.circle, "Ðåæèì: "..(cfg.main.targetState == "player" and "Èãðîêè" or "Òðàíñïîðò"), cx, windowCoordinates[2] - 40, 0xFF84A6E7)
				renderFontDrawTextCenter(fonts1.circle, "Ðàçìåð: "..(cfg.main.circleRadius < maxRad and cfg.main.circleRadius > 30 and cfg.main.circleRadius or (cfg.main.circleRadius <= 30 and "Ìèíèìóì" or (cfg.main.circleRadius >= maxRad and "Ìàêñèìóì"))), windowCoordinates[1] / 2, windowCoordinates[2] - 29, 0xFF84A6E7)
				renderFontDrawTextCenter(fonts1.circle, "Èçìåíåíèå ðàäèóñà: êîë¸ñèêî ìûøè", cx, windowCoordinates[2] - 18, 0xFF84A6E7)
				if cfg.main.targetState == "player" then 
					...
					if doesCharExist(nearPed) then
						...
						if isCharOnScreen(nearPed) then
							...
							local dist = math.floor(getDistanceBetweenCoords3d(plCoordinates[1], plCoordinates[2], plCoordinates[3], myCoordinates[1], myCoordinates[2], myCoordinates[3]))..' ìåòðîâ'
							renderFontDrawTextCenter(fonts1.circle, dist, cx, cy + cfg.main.circleRadius + 10, ACTIVE_COLOR)
							renderFontDrawText(fonts1.circle, '1 - Òåëåïîðòèðîâàòüñÿ',              cx + cfg.main.circleRadius + 10, cy - 30,    ACTIVE_COLOR)
							renderFontDrawText(fonts1.circle, '2 - Òåëåïîðòèðîâàòü ê ñåáå',         cx + cfg.main.circleRadius + 10, cy - 15,    ACTIVE_COLOR)
							renderFontDrawText(fonts1.circle, '3 - Óéòè â ðåêîí çà èãðîêîì',        cx + cfg.main.circleRadius + 10, cy,         ACTIVE_COLOR)
							renderFontDrawText(fonts1.circle, '4 - Âûäàòü 100 HP',                  cx + cfg.main.circleRadius + 10, cy + 15,    ACTIVE_COLOR)
							renderFontDrawText(fonts1.circle, '5 - Çàñïàâíèòü èãðîêà',              cx + cfg.main.circleRadius + 10, cy + 30,    ACTIVE_COLOR)
							renderFontDrawText(fonts1.circle, '6 - Îòêðûòü ìåíþ âçàèìîäåéñòâèÿ ñ èãðîêîì',        cx + cfg.main.circleRadius + 10, cy + 45,    ACTIVE_COLOR)
							...
							if isKeyJustPressed(VK_2) then 
								if cfg.main.mylvladmin >= 2 then 
									...
									sampAddChatMessage(tag..'Âàø àäìèí-óðîâåíü ñëèøêîì íèçîê äëÿ âûïîëíåíèÿ äàííîãî äåéñòâèÿ.', -1)
								end
							end
...
ev.onPlayerDeathNotification = function(killerId, killedId, reason)
		...
		if not isGamePaused() then
		sampAddChatMessage('{E78284}<WARNING TK> {'..color1..'}'..sampGetPlayerNickname(killerId) .. ' [' .. killerId .. '] {84A6E7}óáèë ñâîåãî íàïàðíèêà ïî îðãàíèçàöèè {'..color1..'}'..sampGetPlayerNickname(killedId) .. ' [' .. killedId .. ']', -1)
		end
		end
		end
		end
...
function reconusers()
  ...
  imgui.Begin(fa.ICON_FA_EYE .. u8' Ñëåæêà',reconusers_state,imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize + imgui.WindowFlags.NoMove + imgui.WindowFlags.NoSavedSettings + imgui.WindowFlags.AlwaysAutoResize + imgui.WindowFlags.NoTitleBar)
  imgui.ShowCursor = true
...
local result, x, y, z = getNearestRoadCoordinates()
		if result then
			...
			if not ReconWindow.v then
				...
				sampAddChatMessage(("[AdminTools] Âû òåëåïîðòèðîâàíû íà áëèæàéøóþ îò Âàñ äîðîãó (%dm.)"):format(dist), 0xAAFFAA)
			else
			lua_thread.create(function()
			sampSendChat('/reoff')  
			wait(500)
			setCharCoordinates(PLAYER_PED, x, y, z + 1)
			sampAddChatMessage(("[AdminTools] Âû òåëåïîðòèðîâàëè èãðîêà èç ñëåæêè íà áëèæàéøóþ îò Âàñ äîðîãó (%dm.)"):format(dist), 0xAAFFAA)
			wait(500)
			sampSendChat('/gethere '..spec_id)
			end)
			end
...
function checkKey()
sampAddChatMessage(tag..'/'..klavisha..' {FF69B4}- Îòêðûòü ìåíþ íàñòðîåê ñêðèïòà', -1)
sampAddChatMessage(tag..'/helpa {FF69B4}- Ñïèñîê êîìàíä ñêðèïòà', -1)
sampAddChatMessage(tag..'Ðàçðàáîò÷èê: {FF69B4}Èêîíêà Ëóàñ *ikonka luac', -1)
sampAddChatMessage(tag..'Äëÿ ïîëó÷åíèÿ äîïîëíèòåëüíîé èíôîðìàöèè ïî ñêðèïòó, ïîñåòèòå ñàéò: {FF69B4}blast.hk/members/542334/', -1)
sampAddChatMessage(tag..'Ýòîò ñêðèïò ðàáîòàåò òîëüêî íà: {FF69B4}ARIZONA RP', -1)
end
